...
<div class="movement-controls-section">
    ...
    @php
        ...
        if (isset($nextLocation) && !empty($nextLocation)) {
            $nextName = is_array($nextLocation)
                ? ($nextLocation['name'] ?? null)
                : ($nextLocation->name ?? null);
            $hasNextName = !empty($nextName);
        }
    @endphp
    @if($hasNextName)
        <div class="next-location hidden" id="next-location-info">
            <div class="next-location-header">
                <h4>次の場所</h4>
                <p class="destination-name">{{ is_array($nextLocation) ? ($nextLocation['name'] ?? '') : ($nextLocation->name ?? '') }}</p>
            </div>
            <button class="btn btn-success btn-large" id="move-to-next">
                <span class="btn-icon">🚀</span>
                {{ is_array($nextLocation) ? ($nextLocation['name'] ?? '') : ($nextLocation->name ?? '') }}に移動
            </button>
        </div>
    @endif
