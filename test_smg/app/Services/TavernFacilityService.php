<?php

namespace App\Services;

use App\Models\TownFacility;
use App\Models\Player;
use App\Enums\FacilityType;

class TavernFacilityService extends AbstractFacilityService
{
    // 回復料金の基本レート（後で調整可能）
    private const HP_RATE = 10; // 1HP = 10G
    private const MP_RATE = 15; // 1MP = 15G
    private const SP_RATE = 5;  // 1SP = 5G

    public function __construct()
    {
        parent::__construct(FacilityType::TAVERN);
    }

    public function getAvailableServices(TownFacility $facility): array
    {
        $config = $facility->facility_config ?? [];
        
        return [
            'heal_hp' => [
                'name' => 'HP回復',
                'description' => 'HPを回復します',
                'rate' => $config['hp_rate'] ?? self::HP_RATE,
                'unit' => 'G/HP',
            ],
            'heal_mp' => [
                'name' => 'MP回復',
                'description' => 'MPを回復します',
                'rate' => $config['mp_rate'] ?? self::MP_RATE,
                'unit' => 'G/MP',
            ],
            'heal_sp' => [
                'name' => 'SP回復',
                'description' => 'SPを回復します',
                'rate' => $config['sp_rate'] ?? self::SP_RATE,
                'unit' => 'G/SP',
            ],
            'heal_all' => [
                'name' => '全回復',
                'description' => 'HP、MP、SPを全回復します',
                'rate' => 'variable',
                'unit' => 'G',
                'discount' => $config['full_heal_discount'] ?? 0.1,
            ],
        ];
    }

    public function processTransaction(TownFacility $facility, Player $player, array $data): array
    {
        if (!$this->validateTransactionData($data)) {
            return $this->createErrorResponse('無効なリクエストです。');
        }

        $serviceType = $data['service_type'];
        $amount = $data['amount'] ?? null;
        $config = $facility->facility_config ?? [];

        switch ($serviceType) {
            case 'heal_hp':
                return $this->healHP($player, $amount, $config);
            case 'heal_mp':
                return $this->healMP($player, $amount, $config);
            case 'heal_sp':
                return $this->healSP($player, $amount, $config);
            case 'heal_all':
                return $this->healAll($player, $config);
            default:
                return $this->createErrorResponse('不明なサービスです。');
        }
    }

    public function validateTransactionData(array $data): bool
    {
        return isset($data['service_type']) && 
               in_array($data['service_type'], ['heal_hp', 'heal_mp', 'heal_sp', 'heal_all']);
    }

    private function healHP(Player $player, ?int $amount, array $config): array
    {
        $maxHP = $player->max_hp;
        $currentHP = $player->hp;
        $missingHP = $maxHP - $currentHP;

        if ($missingHP <= 0) {
            return $this->createErrorResponse('HPは既に満タンです。');
        }

        $healAmount = $amount ? min($amount, $missingHP) : $missingHP;
        $rate = $config['hp_rate'] ?? self::HP_RATE;
        $cost = $healAmount * $rate;

        if ($player->gold < $cost) {
            return $this->createErrorResponse("お金が足りません。必要金額: {$cost}G");
        }

        $player->hp += $healAmount;
        $player->gold -= $cost;
        $player->save();

        return $this->createSuccessResponse(
            "HPを{$healAmount}回復しました。",
            [
                'healed_hp' => $healAmount,
                'cost' => $cost,
                'current_hp' => $player->hp,
                'current_gold' => $player->gold,
            ]
        );
    }

    private function healMP(Player $player, ?int $amount, array $config): array
    {
        $maxMP = $player->max_mp;
        $currentMP = $player->mp;
        $missingMP = $maxMP - $currentMP;

        if ($missingMP <= 0) {
            return $this->createErrorResponse('MPは既に満タンです。');
        }

        $healAmount = $amount ? min($amount, $missingMP) : $missingMP;
        $rate = $config['mp_rate'] ?? self::MP_RATE;
        $cost = $healAmount * $rate;

        if ($player->gold < $cost) {
            return $this->createErrorResponse("お金が足りません。必要金額: {$cost}G");
        }

        $player->mp += $healAmount;
        $player->gold -= $cost;
        $player->save();

        return $this->createSuccessResponse(
            "MPを{$healAmount}回復しました。",
            [
                'healed_mp' => $healAmount,
                'cost' => $cost,
                'current_mp' => $player->mp,
                'current_gold' => $player->gold,
            ]
        );
    }

    private function healSP(Player $player, ?int $amount, array $config): array
    {
        $maxSP = $player->max_sp;
        $currentSP = $player->sp;
        $missingSP = $maxSP - $currentSP;

        if ($missingSP <= 0) {
            return $this->createErrorResponse('SPは既に満タンです。');
        }

        $healAmount = $amount ? min($amount, $missingSP) : $missingSP;
        $rate = $config['sp_rate'] ?? self::SP_RATE;
        $cost = $healAmount * $rate;

        if ($player->gold < $cost) {
            return $this->createErrorResponse("お金が足りません。必要金額: {$cost}G");
        }

        $player->sp += $healAmount;
        $player->gold -= $cost;
        $player->save();

        return $this->createSuccessResponse(
            "SPを{$healAmount}回復しました。",
            [
                'healed_sp' => $healAmount,
                'cost' => $cost,
                'current_sp' => $player->sp,
                'current_gold' => $player->gold,
            ]
        );
    }

    private function healAll(Player $player, array $config): array
    {
        $missingHP = $player->max_hp - $player->hp;
        $missingMP = $player->max_mp - $player->mp;
        $missingSP = $player->max_sp - $player->sp;

        if ($missingHP <= 0 && $missingMP <= 0 && $missingSP <= 0) {
            return $this->createErrorResponse('HP、MP、SPは既に満タンです。');
        }

        $hpRate = $config['hp_rate'] ?? self::HP_RATE;
        $mpRate = $config['mp_rate'] ?? self::MP_RATE;
        $spRate = $config['sp_rate'] ?? self::SP_RATE;
        $discount = $config['full_heal_discount'] ?? 0.1;

        $totalCost = ($missingHP * $hpRate) + 
                     ($missingMP * $mpRate) + 
                     ($missingSP * $spRate);

        // 全回復割引適用
        $discountedCost = (int) ($totalCost * (1 - $discount));

        if ($player->gold < $discountedCost) {
            return $this->createErrorResponse("お金が足りません。必要金額: {$discountedCost}G");
        }

        $player->hp = $player->max_hp;
        $player->mp = $player->max_mp;
        $player->sp = $player->max_sp;
        $player->gold -= $discountedCost;
        $player->save();

        return $this->createSuccessResponse(
            "HP、MP、SPを全回復しました。",
            [
                'healed_hp' => $missingHP,
                'healed_mp' => $missingMP,
                'healed_sp' => $missingSP,
                'cost' => $discountedCost,
                'original_cost' => $totalCost,
                'discount_applied' => $totalCost - $discountedCost,
                'current_hp' => $player->hp,
                'current_mp' => $player->mp,
                'current_sp' => $player->sp,
                'current_gold' => $player->gold,
            ]
        );
    }
}