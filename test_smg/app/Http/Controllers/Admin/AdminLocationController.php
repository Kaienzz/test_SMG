...
class AdminLocationController extends AdminController
{
    ...
    public function __construct(AdminAuditService $auditService, MonsterConfigService $monsterConfigService)
    {
        ...
        $this->configService = new LocationConfigService();
        ...
        try {
            ...
            $data = [
                'stats' => [
                    ...
                    'total_connections' => $this->calculateTotalConnections($config),
                ],
                'recent_backups' => $this->getRecentBackups(),
                'config_status' => [
                    'file_exists' => $this->configService->configExists(),
                    'file_size' => $this->getConfigFileSize(),
                    'last_modified' => $config['last_updated'] ?? null,
            ...
            return view('admin.locations.index', [
                'error' => 'ロケーション設定の読み込みに失敗しました: ' . $e->getMessage()
            ]);
        }
    }
    ...
    public function pathways(Request $request)
    {
        ...
        try {
            ...
            if (!empty($filters['search'])) {
                $pathways = array_filter($pathways, function($pathway, $pathwayId) use ($filters) {
                    return stripos($pathway['name'], $filters['search']) !== false ||
                           stripos($pathway['description'] ?? '', $filters['search']) !== false ||
            ...
            if (!empty($filters['category'])) {
                $pathways = array_filter($pathways, function($pathway) use ($filters) {
                    return ($pathway['category'] ?? '') === $filters['category'];
                });
            }
            ...
            if (!empty($filters['type'])) {
                $pathways = array_filter($pathways, function($pathway) use ($filters) {
                    return ($pathway['dungeon_type'] ?? '') === $filters['type'] ||
                           ($pathway['category'] === 'road' && $filters['type'] === 'road');
                });
            }
            ...
            if (!empty($filters['difficulty'])) {
                $pathways = array_filter($pathways, function($pathway) use ($filters) {
                    return ($pathway['difficulty'] ?? '') === $filters['difficulty'];
                });
            }
