...
class PlayerController extends Controller
{
    ...
    public function restoreSp(Request $request): JsonResponse
    {
        $request->validate([
            'amount' => 'integer|min:1|max:1000'
        ]);
        ...
        $oldSp = $player->sp;
        $player->restoreSP($amount);
        $actualRestore = $player->sp - $oldSp;
        $player->save();

        return response()->json([
            'success' => true,
            'message' => "SPが{$actualRestore}回復しました",
            'player' => $player->getStatusSummary(),
            'character' => $player->getStatusSummary(), // 下位互換性のためのalias
        ]);
    }
    ...
    public function gainExperience(Request $request): JsonResponse
    {
        ...
        $result = $player->gainExperience($amount);

        $message = "{$amount}の経験値を獲得しました";
        if ($result['leveled_up']) {
            $message .= "。レベルが{$result['new_level']}に上がりました！";
        }
