...
use App\Domain\Player\PlayerSkills;
use App\Domain\Player\PlayerEquipment;
use App\Domain\Player\PlayerInventory;

class Player extends Model
{
    use PlayerSkills, PlayerEquipment, PlayerInventory;
    ...
    public static function createNewPlayer(int $userId, string $name = '冒険者'): self
    {
        return self::create([
            'user_id' => $userId,
            'name' => $name,
    ...
    {
        return $this->hasMany(ActiveEffect::class, 'player_id');
    }
    ...
    public function calculatePlayerLevel(): int
    {
        // スキルレベルの合計からプレイヤーレベルを計算
        $totalSkillLevel = $this->getTotalSkillLevel();
        
        // スキルレベル合計を基にプレイヤーレベルを計算
        // 例: スキルレベル合計10でプレイヤーレベル2、20で3、など
    ...
    public function updatePlayerLevel(): bool
    {
        $newLevel = $this->calculatePlayerLevel();
    ...
    /**
     * プレイヤーが作成したカスタムアイテム一覧を取得
     */
    public function getCreatedCustomItems(): \Illuminate\Database\Eloquent\Collection
    {
        return $this->customItems()->orderBy('created_at', 'desc')->get();
    }

    /**
     * プレイヤーが作成したカスタムアイテム数を取得
     */
    public function getCustomItemsCount(): int
    {
        return $this->customItems()->count();
    }
}