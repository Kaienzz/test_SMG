...
/**
 * Playerクラスのスキル関連機能を分離したTrait
 * 
 * Phase 2: Character→Player移行によりスキルシステムをPlayer向けに適合
 * スキルシステムの複雑性をPlayerクラスから切り離し
 */
trait PlayerSkills
{
    // キャッシュ用プロパティ
    ...
    {
        return $this->hasMany(Skill::class, 'player_id');
    }
    ...
    public function learnSkill(string $skillType, string $skillName, array $effects = [], int $spCost = 10, int $duration = 5): Skill
    {
        ...
        // 新しいスキルを作成（player_id使用）
        $skill = Skill::createForPlayer($this->id, $skillType, $skillName, $effects, $spCost, $duration);
