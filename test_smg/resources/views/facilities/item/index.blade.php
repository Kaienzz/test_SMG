...
<div id="purchase-tab" class="tab-content bg-white rounded-b-lg shadow-lg p-6">
    ...
    @if(count($facilityData['services']['items']) > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach($facilityData['services']['items'] as $facilityItem)
                <div class="border rounded-lg p-4 hover:shadow-md transition-shadow">
                    <div class="flex justify-between items-start mb-3">
                        <div>
                            <h3 class="font-bold text-lg text-gray-800">{{ $facilityItem['item']['name'] }}</h3>
                            <p class="text-sm text-gray-600">{{ $facilityItem['item']['description'] }}</p>
                        </div>
                        <div class="text-right">
                            <div class="text-lg font-bold text-blue-600">{{ $facilityItem['price'] }}G</div>
                            @if($facilityItem['stock'] !== -1)
                                <div class="text-sm text-gray-500">
                                    在庫: {{ $facilityItem['stock'] }}
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <!-- アイテム効果表示 -->
                    @if($facilityItem['effects'])
                        <div class="mb-3">
                            <div class="text-xs text-gray-500 mb-1">効果:</div>
                            <div class="text-sm text-green-600">
                                @foreach($facilityItem['effects'] as $effect => $value)
                                    @if($effect === 'heal_hp')
                                        HP回復 +{{ $value }}
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-2">
                            <button type="button" 
                                    ...
                                    onclick="changeQuantity({{ $facilityItem['id'] }}, -1)">
                                -
                            </button>
                            <input type="number" 
                                   id="quantity-{{ $facilityItem['id'] }}" 
                                   value="1" 
                                   min="1" 
                                   max="99" 
                        ...
                        <button type="button" 
                                ...
                                onclick="purchaseItem({{ $facilityItem['id'] }})"
                                @if(!$facilityItem['is_in_stock']) disabled @endif>
                            @if($facilityItem['is_in_stock'])
                                購入
                            @else
                                売切
                            @endif
...
function sellItem(slotIndex) {
    ...
    fetch('/facilities/item/transaction', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            ...
            // 所持金更新
            updatePlayerGold(data.player.remaining_gold);
...
function changeQuantity(facilityItemId, change) {
    const input = document.getElementById(`quantity-${facilityItemId}`);
    const currentValue = parseInt(input.value) || 1;
    const newValue = Math.max(1, Math.min(99, currentValue + change));
    input.value = newValue;
}

function purchaseItem(facilityItemId) {
    const quantity = parseInt(document.getElementById(`quantity-${facilityItemId}`).value) || 1;
    
    const formData = new FormData();
    formData.append('facility_item_id', facilityItemId);
    formData.append('quantity', quantity);
    formData.append('_token', '{{ csrf_token() }}');
    ...
    .then(data => {
        if (data.success) {
            ...
            // 在庫更新（必要に応じて）
            if (data.facility_item.remaining_stock !== -1 && !data.facility_item.is_in_stock) {
                const button = document.querySelector(`button[onclick="purchaseItem(${facilityItemId})"]`);
                button.textContent = '売切';
                button.disabled = true;
            }
            
            // 数量を1にリセット
            document.getElementById(`quantity-${facilityItemId}`).value = 1;
