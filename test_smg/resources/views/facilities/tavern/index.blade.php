...
<div class="bg-white rounded-lg shadow-lg p-6">
    ...
    <div class="space-y-4">
        ...
        <!-- 全回復サービス -->
        @if(isset($facilityData['services']['heal_all']))
            @php
                // 全回復の料金を計算
                ...
                $hpRate = $facilityData['services']['heal_hp']['rate'] ?? 10;
                $mpRate = $facilityData['services']['heal_mp']['rate'] ?? 15;
                $spRate = $facilityData['services']['heal_sp']['rate'] ?? 5;
                $discount = $facilityData['services']['heal_all']['discount'] ?? 0.1;
                
                $totalCost = ($missingHP * $hpRate) + ($missingMP * $mpRate) + ($missingSP * $spRate);
                $discountedCost = (int) ($totalCost * (1 - $discount));
            @endphp
            <div class="border rounded-lg p-4 hover:shadow-md transition-shadow bg-white">
                <div class="flex justify-between items-start mb-3">
                    <div>
                        <h3 class="font-bold text-lg text-gray-800">{{ $facilityData['services']['heal_all']['name'] }}</h3>
                        <p class="text-sm text-gray-600">{{ $facilityData['services']['heal_all']['description'] }}</p>
                        @if($discount > 0)
                            <p class="text-sm text-green-600">全回復割引: {{ number_format($discount * 100, 1) }}% OFF</p>
                        @endif
                    </div>
                    <div class="text-right">
                        <div class="text-lg font-bold text-green-600">{{ $discountedCost }}G</div>
                        @if($totalCost != $discountedCost)
                            <div class="text-sm text-gray-500 line-through">{{ $totalCost }}G</div>
                        @endif
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-2">
                        <label class="text-sm text-gray-600">
                            @if($discountedCost == 0)
                                回復の必要はありません
                            @elseif(($player->gold ?? 1000) >= $discountedCost)
                    ...
                    <button type="button" 
                            class="heal-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded font-bold {{ $discountedCost == 0 || ($player->gold ?? 1000) < $discountedCost ? 'opacity-50 cursor-not-allowed' : '' }}"
                            onclick="healService('heal_all')"
                            @if($discountedCost == 0 || ($player->gold ?? 1000) < $discountedCost) disabled @endif>
                        全回復
                    </button>
                </div>
...
<script>
const HP_RATE = {{ $facilityData['services']['heal_hp']['rate'] ?? 10 }};
const MP_RATE = {{ $facilityData['services']['heal_mp']['rate'] ?? 15 }};
const SP_RATE = {{ $facilityData['services']['heal_sp']['rate'] ?? 5 }};
...
function showFullHealDetails() {
    ...
    const discount = {{ $facilityData['services']['heal_all']['discount'] ?? 0.1 }};
    ...
    document.getElementById('cost-heal_all').innerHTML = `
        ...
        <div class="font-bold text-gray-600 mt-1">小計: ${totalCost}G</div>
        ${discount > 0 ? `<div class="text-green-600">割引 (-${Math.floor(discount * 100)}%): -${totalCost - discountedCost}G</div>` : ''}
        <div class="font-bold text-gray-800 mt-1">合計費用: ${discountedCost}G</div>
    `;
}
...
function healService(serviceType) {
    ...
    fetch('/facilities/tavern/transaction', {
        method: 'POST',
        body: formData
    })
