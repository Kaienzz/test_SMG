{{-- Dungeon State - Left Area: Merged Right + Left Content --}}
...
<div class="movement-controls-section">
    ...
    <div class="movement-controls hidden" id="movement-controls">
        <button class="btn btn-warning movement-btn" id="move-north" onclick="move('north')" data-direction="north">
            ...
            <span class="btn-text">奥に進む（探索）</span>
        </button>
        <button class="btn btn-warning movement-btn" id="move-south" onclick="move('south')" data-direction="south">
            <span class="btn-icon">⬇️</span>
            <span class="btn-text">入口に戻る（後退）</span>
        </button>
    </div>

    {{-- Next Location (only show when at dungeon boundaries) --}}
    @php
        ...
        // 改善: 境界位置にいる場合、利用可能な接続を直接チェック
        if ($atBoundary && isset($availableConnections) && is_array($availableConnections)) {
            foreach ($availableConnections as $conn) {
                // 接続データの詳細を取得
                $t = $conn['target_location'] ?? null;
                $connName = null;
                
                if (is_array($t)) {
                    $connName = $t['name'] ?? null;
                } elseif (is_object($t)) {
                    $connName = $t->name ?? null;
                }
        ...
        // フォールバック: コントローラからの nextLocation （既存のロジック）
        if (!$showNextLocation && isset($nextLocation) && !empty($nextLocation)) {
            $nextName = is_array($nextLocation) ? ($nextLocation['name'] ?? null) : ($nextLocation->name ?? null);
            if (!empty($nextName) && $atBoundary) {
                // nextLocation があるが connection ID がない場合、availableConnections から検索
                if (isset($availableConnections) && is_array($availableConnections)) {
                    foreach ($availableConnections as $conn) {
                        $t = $conn['target_location'] ?? null;
                        $connName = is_array($t) ? ($t['name'] ?? null) : (is_object($t) ? ($t->name ?? null) : null);
                        if ($connName === $nextName) {
                            $nextConnId = $conn['id'] ?? null;
                            $showNextLocation = true;
                            break;
                        }
    ...
    <div class="movement-status">
        <div class="status-item">
            <span class="status-label">探索可能歩数:</span>
            <span class="status-value" id="available-steps">0</span>
        </div>
        <div class="status-item">
            <span class="status-label">探索方向:</span>
...
@php
    $gatheringSkill = null;
    $currentEnvironment = $currentLocation->category ?? 'dungeon';
